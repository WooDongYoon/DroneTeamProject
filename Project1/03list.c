...
{
	listNode* p;
	int Alpha=65;	//알파벳 출력시 사용할 변수
	double total_distance=0;	//지점들의 거리의 합
	double total_battery=0;
	p = L->head;
	while (p != NULL) 
	{
		printf("%c%c\t\t %.1f\t %.1f\t %.1f\n",Alpha, Alpha+1, p->distance, p->k, p-> battery_used);
		fprintf(fp,"%.1f\t %.1f\t %.1f\n", p->distance, p->k, p->battery_used);
...
// 마지막 노드로 삽입하는 연산 
void insertLastNode(linkedList_h* L, double distance, double k, double battery_used) 
{
	listNode* newNode;
	listNode* temp;
